...
export class Calculus{
     ...
     calculateCloseness(conocimientoEvaluado,baseConocimiento,memoriaDeTrabajo, minimumPorcentage){
       ...
       baseConocimiento.forEach(element => {
         ...
         if(percentage > minimumPorcentage && percentage != 100){
           let showPercentage = percentage*0.01;
           let closeness = {padecimiento: element.partesConclusion[0].desc,  id: element.partesConclusion[0].padecimiento, porcentaje: Math.floor(percentage), showPorcentaje: showPercentage.toFixed(1)};
